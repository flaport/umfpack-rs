...
fn example4_2_rs() {
    ...
    let Ap = vec![0, 2, 5, 9, 10, 12];
    let Ai = vec![0, 1, 0, 2, 4, 1, 2, 3, 4, 2, 1, 4];
    let Ax = vec![2.0, 1.0, 3.0, 1.0, 3.0, 1.0, -1.0, 1.0, 4.0, 1.0, 4.0, 1.0, -3.0, 1.0, 1.0, 1.0, 2.0, 1.0, 2.0, 1.0, 6.0, 1.0, 1.0, 1.0];
    ...
    let control = Control::new();
    let mut symbolic = Symbolic::new();
    umfpack_zi_symbolic(n, n, &Ap, &Ai, &Ax, None, &mut symbolic, &control, &mut info);

    let mut numeric = Numeric::new();
    umfpack_zi_numeric(&Ap, &Ai, &Ax, None, &symbolic, &mut numeric, &control, &mut info);

    umfpack_zi_solve(UMFPACK::A, &Ap, &Ai, &Ax, None, &mut X, None, &B, None, &numeric, &control, &mut info);
